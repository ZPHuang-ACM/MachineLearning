...
% State is angle -- play with these for better results. Faster convergence
% with rough discretization, less jittery with fine.
x1 = -pi:0.05:pi;
%State2 angular rate
x2 = -pi:0.1:pi;

%Generate a state list and initialize the state matrix with all possible state combinations
...
% check the reinforcement learning MATLAB lecture 3 Q-Table for more info
...
Vorig = reshape(max(Q,[],2),[length(x2),length(x1)]); % x2 in rows, x1 in columns, max(Q,[],2) finds the max on each row of Q, then reshape the vector
...
f = plot(0,0,'b','LineWidth',10); % Pendulum stick in blue
axPend = f.Parent;
...
colormap('hot'); % colormap map sets the colormap for the current figure to one of the predefined colormaps
subplot(1,4,[2:4]);
hold on
map = imagesc(reshape(R,[length(x2),length(x1)])); % Display image with scaled colors
axMap = map.Parent;
...
axMap.Box = 'off';
axMap.FontSize = 14;
caxis([3*min(R),max(R)]) % sets the colormap limits for the current axes. limits is a two-element vector of the form [cmin cmax]
pathmap = plot(NaN,NaN,'.g','MarkerSize',30); % The green marker that travels through the state map to match the pendulum animation
map.CData = V; % Image color data, specified in one of these forms: Vector or matrix — This format defines indexed image data.
hold off

%% Start learning!

% Number of episodes or "resets" (2000)
for episodes = 1:maxEpi
    
    z1 = startPt; % Reset the pendulum on new episode.
    
    % Number of actions we're willing to try before a reset (1500)
    for g = 1:maxit
        ...
        if (rand()>epsilon || episodes == maxEpi) && rand()<=successRate % Pick according to the Q-matrix it's the last episode or we succeed with the rand()>epsilon check. Fail the check if our action doesn't succeed (i.e. simulating noise)
            ...
            aIdx = randi(length(actions),1); % Random action; randi returns a random integer from 1:length(actions)
        end
        ...
        % RK4 Loop - Numerical integration (Fouth Order Runge-Kutta Algorithm)
        for i = 1:substeps
            k1 = Dynamics(z1,T);
